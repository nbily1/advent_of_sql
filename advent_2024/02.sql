...
-- SQL
select string_agg(ltr, '' order by id) as output_str
from (
  select a.*, chr(value) as ltr from letters_a a
  union
  select b.*, chr(value) as ltr from letters_b b
) sub
where substring(ltr, '[A-Za-z !"''(),.:;?-]', '') <> ''
order by 1